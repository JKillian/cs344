                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  if( xPos < numCols && yPos < numRows)
  {
    ...
    outputChannel[index] = (unsigned char)blurredPixel;
  }
}
                                ...
                                const float* const h_filter, const size_t filterWidth)
{
  ...
  // allocate memory for filter on device and then copy host filter to it
  const int filterMemSize = sizeof(float) * filterWidth * filterWidth;
  checkCudaErrors(cudaMalloc((void **) &d_filter, filterMemSize));
...
{
  //number of threads in the x and y dimension of each block
  const int BLOCK_DIM = 16;

  const dim3 blockSize(BLOCK_DIM, BLOCK_DIM, 1);

  const dim3 gridSize(((numCols-1)/BLOCK_DIM) + 1, ((numRows-1)/BLOCK_DIM) + 1, 1);
